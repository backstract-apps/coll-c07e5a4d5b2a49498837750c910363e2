...
async def put_items_id(db: Session, raw_data: schemas.PutItemsId):
    ...
    if items_edited_record:
        for key, value in {'id': id, 'content': content, 'created_at': created_at}.items():
            setattr(items_edited_record, key, value)

        db.commit()
        db.refresh(items_edited_record)

        items_edited_record = items_edited_record.to_dict() if hasattr(items_edited_record, 'to_dict') else vars(items_edited_record)
...
async def get_items(db: Session):
    ...
    items_all = query.all()
    items_all = [new_data.to_dict() for new_data in items_all] if items_all else items_all
